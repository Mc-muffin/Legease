<template>
  <CRow>
    <CCol col="12">
      <CCard>
        <CCardHeader>
          <CIcon name="cil-grid" />Lista de usuarios
          <CButton
            size="sm"
            @click="createUser"
            class="float-right"
        ...
        <CCardBody>
          ...
          >
            <template #title="{item}">
              <td>{{item.title ? item.title.name : 'Sin definir'}}</td>
            </template>
            <template #department="{item}">
              <td>{{item.department ? item.department.name : 'Sin definir'}}</td>
            </template>
          </CDataTable>
          <CPagination
            align="center"
  ...
  methods: {
    ...
    upload(files) {
      ...
      services.user
        .import(formData)
        .then(response => {
          this.$toast.success("Ingresado con éxito");
          this.fetch();
        })
        .catch(error => {
